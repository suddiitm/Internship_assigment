...
router.post('/calculate-distribute-produce-share', async (req, res) => {
  ...
  try {
    ...
    const [treeAdoptions] = await connection.execute(
      'SELECT ta.id AS adoption_id, ta.tree_id, p.produce_id, p.quantity, u.id AS user_id ' +
    ...
    for (const adoption of treeAdoptions) {
      ...
      await connection.execute(
        'INSERT INTO user_produce_shares (share_id, user_id, produce_id, share_quantity) VALUES (?, ?, ?, ?)',
        [adoptionId, userId, produceId, sharePercentage]
      );
    }
